...
function RecentServers() {
  ...
  const reload = useCallback(async () => {
    try {
      ...
      if (uuid) {
        ...
        combined.forEach((m) =>
          map.set(m.id, { id: m.id, room_name: m.room_name, created_at: m.created_at })
        );
        (hosted ?? []).forEach((m) => {
          if (!map.has(m.id))
            map.set(m.id, { id: m.id, room_name: m.room_name, created_at: m.created_at });
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      supabase.removeChannel(mmCh);
      window.removeEventListener("meetings-updated", handleLocal as EventListener);
    };
  }, [reload]);
...
function ParticipantsCard({
  ...
  const [participants, setParticipants] = useState<
    Array<{ id: number; nickname: string; accent_color: string }>
  >([]);
    ...
    const load = async () => {
      ...
      try {
        ...
        try {
          ...
          setExpectedCount(n);
          try {
            localStorage.setItem(`home:memberCount:${targetMeeting}`, String(n));
          } catch {
            /* ignore */
          }
        } finally {
        ...
        const { data: usersRows } = await supabase
          .from("users")
          .select("id, user_uuid, name, email")
        ...
        (profiles ?? []).forEach(
          (p: { id: number; nickname: string | null; accent_color: string | null }) =>
            pmap.set(p.id, { nickname: p.nickname, accent_color: p.accent_color })
        );
        const list = (usersRows ?? []).map(
          (u: { id: number; name: string | null; email: string | null }) => {
            ...
            const nickname =
              prof?.nickname && prof.nickname.trim()
                ...
                : u.name && u.name.trim()
                  ...
                  : (u.email ?? `사용자 #${u.id}`);
            const accent =
              prof?.accent_color && /^#([0-9a-fA-F]{6})$/.test(prof.accent_color)
                ? prof.accent_color
                : "#7e22ce";
...
export default function HomePage() {
      ...
      try {
        ...
        if (!profileRow) {
          await supabase
            .from("profile")
            .upsert(
              { id: pk, nickname: displayName, language: "ko", mic_enabled: true },
              { onConflict: "id" }
            );
          ...
          const validColor =
            typeof profileRow.accent_color === "string" &&
            /^#([0-9a-fA-F]{6})$/.test(profileRow.accent_color)
              ? profileRow.accent_color
              : null;
          setProfileColor(validColor);
          const cleanedBio =
            typeof profileRow.bio === "string" && profileRow.bio.trim().length > 0
              ? profileRow.bio
              : null;
      ...
      if (event === "SIGNED_OUT") {
        try {
          ...
          Object.keys(localStorage).forEach((k) => {
            if (k.startsWith("home:memberCount:")) localStorage.removeItem(k);
          });
        } catch {
          /* ignore */
        }
    ...
    const loadFriends = async () => {
      ...
      try {
        ...
        const profileMap = new Map<
          number,
          { nickname: string | null; accent_color: string | null }
        >();
        (profiles ?? []).forEach(
          (p: { id: number; nickname: string | null; accent_color: string | null }) => {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      try {
        localStorage.setItem("home:selectedMeeting", JSON.stringify({ meetingId, meetingName }));
      } catch {
        /* no-op */
      }
    };
    ...
    return () => {
      window.removeEventListener("show-participants", handler as EventListener);
    ...
    async (targetDeviceId?: string) => {
      try {
        ...
        try {
          ...
          setMicLabel(label || "");
          if ((settings as { deviceId?: string }).deviceId)
            setMicDeviceId((settings as { deviceId?: string }).deviceId || null);
        } catch {
          /* ignore */
        }
        const AC =
  ...
  return (
    <div
      className={`bg-gray-50 dark:bg-[#18191c] h-screen overflow-hidden grid grid-cols-1 ${rightPanelWidthClass} ${pagePaddingClass} px-0`}
    >
      {/* 왼쪽: 기존 메인 콘텐츠 */}
      <div>
        ...
        <div className="ml-0 lg:ml-2 mt-2 lg:mt-4">
          <div className="bg-white dark:bg-[#1a1d21] rounded-2xl shadow-xl p-6 mb-6 flex items-center justify-between">
            ...
            <div className="flex flex-col items-end gap-2">
              <Link to="/workspace/profile" className="text-sm text-blue-600 hover:underline">
                편집
              </Link>
            </div>
          </div>
        ...
        {/* 2단 가로 카드 레이아웃 */}
        <div
          className={`grid grid-cols-1 ${up.md ? "md:grid-cols-2" : ""} ${down.md ? "gap-4" : "gap-6"} items-stretch w-full ml-0 lg:ml-2 mt-2 lg:mt-4`}
        >
          {/* 왼쪽 카드: 설명 또는 참여자 */}
          {participantsView ? (
            <ParticipantsCard
            ...
            <div
              ...
              className="bg-white dark:bg-[#1a1d21] rounded-xl shadow-xl p-8 flex flex-col items-start justify-start"
            >
              <h1 className="text-2xl font-bold text-gray-800 dark:text-white mb-2">Loch</h1>
              <p className="text-sm text-gray-500 dark:text-gray-300 mb-6">
                ...
                <br />
                <br />
                만들어진 회의록을 github에 업로드 할 수 있게 하여 회의록을 공유할 수 있게
                하였습니다.
                <br />
                <br />
                봐주셔서 감사합니다!!
          ...
          {/* 오른쪽 카드: 내 친구 미리보기 */}
          <div
            className={`bg-white dark:bg-[#1a1d21] rounded-xl shadow-xl ${down.md ? "p-5" : "p-8"} flex flex-col`}
          >
              ...
              <div className="flex flex-col gap-3">
                ...
                {friends.length > 5 && (
                  <Link
                    ...
                    className="self-end text-xs text-gray-500 dark:text-gray-300 hover:underline"
                  >
                    모두 보기
                  </Link>
                )}
        ...
        <div className="w-full px-0 pt-1">
          <div
            className={`bg-white dark:bg-[#1a1d21] rounded-2xl shadow-xl ${down.md ? "p-4" : "p-6"} ml-0 lg:ml-2 mt-2 lg:mt-4`}
          >
            {/* 상단: 제목 + 버튼 */}
            ...
            <button
              onClick={() => {
                if (isMicTesting) {
                  ...
                  void startMicTest(micDeviceId || undefined);
                }
              }}
              className="inline-flex items-center justify-center border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-1 text-xs hover:bg-gray-100 dark:hover:bg-[#23242e] transition"
            >
              ...
              <div className="flex gap-2 justify-end">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (isMicTesting) {
                      stopMicTest();
                      await startMicTest(pendingDeviceId || undefined);
                    }
                  }}
