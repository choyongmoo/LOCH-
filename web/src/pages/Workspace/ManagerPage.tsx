          ...
          skeletonCount > 0 ? (
            <>
              {Array.from({ length: Math.max(0, skeletonCount) }).map((_, i) => (
                <div
                  key={i}
                  className="flex items-center px-2 py-3 border-b border-gray-200 dark:border-[#23242e]"
                >
                  <div className="w-8" />
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    await supabase
                      ...
                      .eq("user_id", userId);
                    await supabase.from("servers").delete().eq("id", leaveTarget.id);
