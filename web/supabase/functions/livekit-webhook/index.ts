      ...
      case "participant_left":
        {
          ...
          const { data: roomRow } = await supabase.from("rooms").select("id, is_active, user_count").eq("id", roomId).single();
          if (roomRow && roomRow.is_active === true && (roomRow.user_count ?? 0) === 0) {
            try {
              const svc = new RoomServiceClient(Deno.env.get("LIVEKIT_URL") ?? "", Deno.env.get("LIVEKIT_API_KEY") ?? "", Deno.env.get("LIVEKIT_API_SECRET") ?? "");
