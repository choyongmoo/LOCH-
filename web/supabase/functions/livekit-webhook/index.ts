...
import { WebhookReceiver, RoomServiceClient } from "https://esm.sh/livekit-server-sdk";
const supabase = createClient(Deno.env.get("SUPABASE_URL") ?? "", Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? "");
const receiver = new WebhookReceiver(Deno.env.get("LIVEKIT_API_KEY") ?? "", Deno.env.get("LIVEKIT_API_SECRET") ?? "");
const service = new RoomServiceClient(Deno.env.get("LIVEKIT_URL") ?? "", Deno.env.get("LIVEKIT_API_KEY") ?? "", Deno.env.get("LIVEKIT_API_SECRET") ?? "");
